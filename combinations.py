...
def get_combined_cfgs_journal_version():
    ...
    cfg = combinator.get_classifier_technique_config(bow=True, lemmatize=True, remove_stopwords=True)
    key = combinator.get_key_for_classifier_config(cfg)
    toreturn[key] = cfg
    ...
    #Metadata
    cfg = combinator.get_classifier_technique_config(rating=True)
    key = combinator.get_key_for_classifier_config(cfg)
    toreturn[key] = cfg

    cfg = combinator.get_classifier_technique_config(rating=True, length=True)
    key = combinator.get_key_for_classifier_config(cfg)
    toreturn[key] = cfg

    cfg = combinator.get_classifier_technique_config(rating=True, length=True, tense=True)
    key = combinator.get_key_for_classifier_config(cfg)
    toreturn[key] = cfg

    cfg = combinator.get_classifier_technique_config(rating=True, length=True, tense=True, sentiment1=True)
    key = combinator.get_key_for_classifier_config(cfg)
    toreturn[key] = cfg

    cfg = combinator.get_classifier_technique_config(rating=True, length=True, tense=True, sentiment2=True)
    key = combinator.get_key_for_classifier_config(cfg)
    toreturn[key] = cfg
