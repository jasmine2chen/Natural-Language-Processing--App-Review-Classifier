...
class ReviewClassifier (BaseReviewClassifier):
    def __init__(self, csv_prefix, labels, pos_tbl_prefix, neg_tbl_prefix, classifier_cfg, cfg_id, init_trainset, old_traintest_set = False, datafile_suffix="_all"):
        ...
        if "Bug" in labels:
            train_size = data_size-126
            test_size = 126
        if "Feature" in labels:
            train_size = data_size-60
            test_size = 60
        if "UserExperience" in labels:
            train_size = data_size-140
            test_size = 140
        if "Rating" in labels:
            train_size = data_size-336
            test_size = 336
        ...
        for label in labels:
            ...
            filename = '%smeanscores_training_testing.csv' % csv_prefix_tmp
            ...
            filename = '%sprecision_training_testing.csv' % csv_prefix_tmp
            p = pathlib.Path(filename)
            writeheader = not p.exists()
            ...
            filename = '%srecall_training_testing.csv' % csv_prefix_tmp
            p = pathlib.Path(filename)
            writeheader = not p.exists()
            ...
            filename = '%sf1score_training_testing.csv' % csv_prefix_tmp
            p = pathlib.Path(filename)
            writeheader = not p.exists()
        ...
        filename = '%stime_training_testing.csv' % csv_prefix
        p = pathlib.Path(filename)
        writeheader = not p.exists()
